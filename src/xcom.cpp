...
/*
    for(int i=0;i<NENG;i++)
    {
        ...
        if(KZ[i]>0)
        {
            float PHT1 =PHT[i]-PHDIF[i];
            float AT1;
            float ATNC1;
            if(NF==2)
            {
                // AT1=AT[i]-AVOG*PHDIF[i]/ATWT;
                //	ATNC1=ATNC[i]-AVOG*PHDIF[i]/ATWT;
            }else{
                AT1=AT[i]-PHDIF[i];
                ATNC1=ATNC[i]-PHDIF[i];
            }
            //	printf("%d k%10.3E%10.3E%10.3E%10.3E%10.3E%10.3E\n",i,EN[i]/1.E+6,SCTCO[i],SCTIN[i],PHT1,PRAT[i],PREL[i]);
        }
